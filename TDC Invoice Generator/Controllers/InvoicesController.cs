...
using InvoiceGenerator_Core.Helper;
using InvoiceGenerator_Core.Models;
using InvoiceGenerator_Core.ViewModels.Invoices;
...
namespace InvoiceGenerator_Core.Controllers
{
    ...
    public class InvoicesController : Controller
    {
        ...
        public ActionResult Create(CreateInvoiceViewModel invoice)
        {
            ...
            Setting settings = _context.Settings.SingleOrDefault(x => x.Id == Settings.ConfigKey);
            ...
            if (invoice.Status == "DL")
            {
                if (settings != null)
                {
                    ...
                    WaybillNumberHelper.UpdateWaybillNumber(settings.LastWaybillNumber + 1, createdUser);
                }
            }
        ...
        public ActionResult Details(string id)
        {
            ...
            string shouldDownload = Request.QueryString["download"];
        ...
        public ActionResult Edit(EditInvoiceViewModel data)
        {
            ...
            if (invoiceInDb != null)
            {
                ...
                if (data.Status == "DL")
                {
                    ...
                    {
                        // Update Last Waybill Number
                        WaybillNumberHelper.UpdateWaybillNumber(settings.LastWaybillNumber + 1, User.Identity.Name);
                    }
                }
            }
        ...
        public void DownloadInvoiceExcel(string id)
        {
            if (!string.IsNullOrWhiteSpace(id))
            {
...

                {
                    ...
                    if (invoice != null)
                    {
                        FileInfo template = new FileInfo(Server.MapPath(Settings.InvoiceTemplatePath));
                        ...
                        Response.AddHeader("content-disposition",
                            "attachment; filename=" + invoice.GeneralWaybillNumber + invoice.BranchCode + " - " + invoice.Client.Name + " - " +
                            invoice.GeneralWaybillNumber +
                            " - " + dateTime + ".xlsx");
                        Response.BinaryWrite(ep.GetAsByteArray());

                        // Flush the response to send any buffered content to the client
                        Response.Flush();
